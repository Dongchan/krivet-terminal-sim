...
- VSCode를 직접 모방하지 않고 **일반화된 IDE 레이아웃**: 좌측 사이드바(SVG 일반 아이콘), 중앙 에디터, 하단 통합 터미널.
- 워터마크 "교육용 모형 (Educational Mockup)" 항시 표시.
- 시나리오: `code .` 입력 → 화면 전환 애니메이션 → 가짜 파일트리 클릭 → 에디터에 내용 → 내장 터미널에서 `git status` 한 번.
- Terminal 클래스를 그대로 임베드 (재사용성 검증).
